document.getElementById("recipe-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    recipe.steps.forEach((step) => {
        ...
        totalWaterAmount += Number(step.water);
        totalTime += Number(step.duration_sec);
        totalTimeMin = Math.floor(totalTime / 60);
        totalTimeSec = totalTime % 60;
        li.innerHTML = `<span class="step-label">Step${step.step}:</span><br />Water: input ${step.water}ml (total ${totalWaterAmount}ml)<br />Time: ${totalTimeMin}m ${totalTimeSec}s`;
        ul.appendChild(li);
    });
